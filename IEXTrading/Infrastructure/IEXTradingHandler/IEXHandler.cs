            ...
            while (true)
            {
                ...
                if (Count < companies_final.Count())
                {
                    ...
                    if (!string.IsNullOrEmpty(Json_quote))
                    {
                        ...
                        quotes_Name = quotes_Name.Where(x => ((x.Value?.FirstOrDefault().Value?.name_company != "") &&
                        (x.Value?.FirstOrDefault().Value?.week52_high - x.Value?.FirstOrDefault().Value?.week52_low) != 0)
                ...
                foreach (var lq in List_Quote)
                {
                    if (lq.Value != null)
                    {
                        if (lq.Value.FirstOrDefault().Value != null)
                        {
                            ...
                            lq.Value.FirstOrDefault().Value.week_52_Price_Range = (Value_quote.close - Value_quote.week52_low) / (Value_quote.week52_high - Value_quote.week52_low);
                        }
                    }
                }
            }
